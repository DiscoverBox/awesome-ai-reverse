...
Awesome AI Reverse Engineering 是一份围绕 **AI + 逆向工程** 生态整理的工具清单，收录：

* MCP（Model Context Protocol）服务器；
* Claude Code、Cursor、Codex 等平台可使用的 Skill；
* IDA Pro、Ghidra、Binary Ninja、JADX 插件；
* 浏览器自动化和流量分析工具；
* Android、iOS 动态分析工具；
* AI 驱动的完整逆向 Agent 与桌面工作站。

这些项目尝试让大语言模型不再只是“解释一段代码”，而是能够直接参与真实的分析过程：

* 操作浏览器并读取运行时变量；
* 设置断点、注入 Hook、采集调用栈；
* 分析 JavaScript、WASM、Worker 和混淆代码；
* 调用 IDA Pro、Ghidra、Binary Ninja 和 JADX；
* 控制 Android 或 iOS 测试设备；
* 使用 Frida 进行动态插桩；
* 读取和分析 HTTP、HTTPS、WebSocket 流量；
* 生成分析报告、调用链和本地复现代码。
...
* JS Reverse MCP 提供浏览器、网络和运行时能力；
* reverse-skill 提供调用链、混淆和协议分析方法。

---

## 2. 复杂 JavaScript、Worker、WASM 或 JSVMP
